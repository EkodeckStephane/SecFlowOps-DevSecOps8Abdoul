...
from secflowops.normalizer.common import write_json
...
def resolve_tool(name: str) -> str | None:
    ...
    if exe:
        return exe
    local_names = {"gitleaks": ROOT / "tools" / "gitleaks" / "gitleaks.exe"}
...
def run_command(
    cmd: list[str], *, cwd: Path, log_path: Path, timeout: int = 600,
    ...
    try:
        ...
        return {
            "command": cmd, "returncode": 999, "elapsed_seconds": elapsed,
...
def git_commit_or_local(path: Path | None = None) -> str:
    ...
    try:
        proc = subprocess.run(["git", "rev-parse", "HEAD"], cwd=cwd, text=True, capture_output=True)
...
def build_command_for_workspace(workspace: Path, mode: str) -> tuple[list[str], str]:
    if mode == "skip":
        return ["python", "-c", "print('build skipped by experiment configuration')"], "skipped_by_design"
    ...
    return ["python", "-c", "print('no local build command configured')"], "no_native_test_command"
...
def run_zap_scan(workspace: Path, raw_dir: Path, scanner_dir: Path, log_path: Path) -> dict[str, Any]:
    ...
    if not java or not zap_jar:
        return {"command": ["zap"], "returncode": 999, "elapsed_seconds": 0.0, "stderr": "java_or_zap_unavailable"}
    app = workspace / "app.py"
    if not app.exists():
        return {"command": ["zap"], "returncode": 998, "elapsed_seconds": 0.0, "stderr": "no local app.py target for DAST"}
    ...
    server = subprocess.Popen(
        [sys.executable, "app.py"], cwd=workspace, stdout=server_out,
    ...
    try:
        ...
        if port_available(target_port):
            return {"command": ["zap"], "returncode": 996, "elapsed_seconds": 0.0, "stderr": f"target server did not listen on {target_port}"}
        zap_home = raw_dir / "zap_home"
        zap_home.mkdir(parents=True, exist_ok=True)
        zap_out = scanner_dir / "zap.json"
        cmd = [
            java, "-jar", str(zap_jar), "-cmd", "-silent", "-dir", str(zap_home),
            "-port", str(proxy_port), "-quickurl", f"http://127.0.0.1:{target_port}/search?q=secflowops",
            "-quickout", str(zap_out), "-quickprogress",
        ]
        return run_command(cmd, cwd=ROOT, log_path=log_path, timeout=300, env=scanner_environment())
    finally:
        server.terminate()
        try:
...
def run_scanners(
    workspace: Path, raw_dir: Path, log_path: Path, *, enable_zap: bool = False,
    ...
    if trivy:
        ...
        cmd = [
            trivy, "fs", "--skip-version-check", "--skip-db-update", "--skip-java-db-update",
            "--skip-check-update", "--skip-vex-repo-update", "--cache-dir", str(ROOT / "artifact" / "trivy_cache"),
            "--scanners", "vuln,secret,misconfig", "--format", "json", "--output", str(trivy_out), str(workspace),
        ]
    ...
    if semgrep:
        ...
        cmd = [
            semgrep, "scan", "--metrics=off", "--disable-version-check", "--config",
            str(ROOT / "scanners" / "semgrep" / "secflowops.yml"), "--json", "--output", str(semgrep_out), str(workspace),
        ]
    ...
    if gitleaks:
        ...
        cmd = [
            gitleaks, "detect", "--source", str(workspace), "--config",
            str(ROOT / "scanners" / "gitleaks" / "gitleaks.toml"), "--report-format", "json",
            "--report-path", str(gitleaks_out), "--no-git",
        ]
...
def _step_returncodes_ok(step: dict[str, Any]) -> bool:
...
def run_one(
    config: str, *, repo: str, scenario: str, repetition: int,
    campaign_id: str | None = None, build_mode: str = "auto", enable_zap: bool = False,
) -> dict[str, Any]:
    ...
    write_json(remediation_log, {"mode": "not_applicable", "events": [], "remediated_count": 0, "validation": {"validation_executed": False, "validation_status": "not_applicable"}})
    ...
    if config != "C0_BuildOnly":
        ...
        metadata["enabled_tools"] = sorted(scan_results)
    ...
    if config in ("C3_RemediationOnly", "C4_SecFlowOps"):
        ...
        metadata["steps"]["remediation"] = {
            "elapsed_seconds": time.perf_counter() - remediation_started,
            "remediated_count": remediation_result.get("remediated_count", 0),
            "validation_status": (remediation_result.get("validation") or {}).get("validation_status"),
        }
    ...
    policy_required = config in ("C2_PolicyOnly", "C4_SecFlowOps")
    if policy_required:
        policy_started = time.perf_counter()
        decision = evaluate_policy_file(
            residual_path, policy_decision_path,
        ...
        release_decision = "ALLOW" if decision.get("allow") else "DENY"
        release_allowed: bool | None = bool(decision.get("allow"))
    else:
        decision = {"engine": "not_applicable", "allow": None, "deny": [], "warn": [], "summary": {}}
        write_json(policy_decision_path, decision)
        release_decision = "NOT_APPLICABLE"
        release_allowed = None

    elapsed = time.perf_counter() - start
    scanner_steps_ok = all(
        _step_returncodes_ok(step)
        for name, step in metadata["steps"].items()
        if name.startswith("scanners_") and isinstance(step, dict)
    )
    policy_executed = (not policy_required) or metadata.get("steps", {}).get("policy_gate", {}).get("engine") in {"opa", "python_fallback"}
    execution_success = build_result["returncode"] == 0 and scanner_steps_ok and not metadata["tool_failures"] and policy_executed

    metadata["finished_at"] = utc_now()
    metadata["pipeline_time_seconds"] = elapsed
    metadata["execution_success"] = execution_success
    metadata["release_decision"] = release_decision
    metadata["release_allowed"] = release_allowed
    ...
    append_manifest(run_id, config, repo, scenario, repetition, execution_success, release_decision, raw_dir / "metadata.json")
    return metadata


def append_manifest(
    run_id: str, config: str, repo: str, scenario: str, repetition: int,
    execution_success: bool, release_decision: str, metadata_path: Path,
) -> None:
    manifest = ROOT / "experiments" / "raw_logs_manifest.csv"
    with manifest.open("a", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            run_id, config, repo, scenario, repetition,
            "completed" if execution_success else "execution_failed",
            release_decision, metadata_path, "local",
        ])
    ...
    for rep in range(1, args.repetitions + 1):
        for repo in repos:
            for config in args.configs:
                ...
                rows.append(run_one(
                    config, repo=repo, scenario=args.scenario, repetition=rep,
                    campaign_id=args.campaign_id, build_mode=args.build_mode, enable_zap=args.enable_zap,
                ))
    print(json.dumps({"runs": len(rows), "completed": sum(1 for r in rows if r["execution_success"])}, indent=2))
